...
const AIR_CONDITIONER_OUTDOOR_IMAGE = '/assets/images/products/air-conditioner-outdoor-unit.png';

// Các mẫu treo tường dùng thay cho nhóm máy tủ đứng cũ.
const WALL_AIR_CONDITIONER_IMAGES = [
    '/assets/images/products/gia-khang/073-may-lanh-mitsubishi-electric-1-5-hp-ms-js35vf.jpg',
    '/assets/images/products/gia-khang/074-may-lanh-sharp-inverter-1-5-hp-ah-x13cew.jpg',
    '/assets/images/products/gia-khang/078-may-lanh-samsung-inverter-1-5-hp-ar60h13d1mwnsv.webp',
    '/assets/images/products/gia-khang/079-may-lanh-tcl-inverter-2-hp-tac-sa18csv-za.jpg',
    '/assets/images/products/gia-khang/090-may-lanh-daikin-inverter-3-hp-ftkz71vvmv.jpg',
    '/assets/images/products/gia-khang/103-may-lanh-panasonic-inverter-2-5-hp-cu-cs-xu24bkh-8.jpg'
];
...
// Hiển thị nhóm tủ đứng cũ thành máy lạnh treo tường.
function normalizeAirConditionerProduct(product) {
    const name = String(product.name || '');
    if (!/máy lạnh tủ đứng/i.test(name)) return product;

    const imageIndex = Math.abs(Number(product.id) || 0) % WALL_AIR_CONDITIONER_IMAGES.length;
    return {
        ...product,
        name: name.replace(/Máy lạnh tủ đứng/i, 'Máy lạnh treo tường'),
        category: 'Điều Hòa',
        image: WALL_AIR_CONDITIONER_IMAGES[imageIndex],
        specs: String(product.specs || '').replace(/Tủ đứng/gi, 'Treo tường')
    };
}
...
async function loadProductsFromAPI() {
    try {
        ...
        if (data.success) {
            products = (data.products || []).map(p => normalizeAirConditionerProduct({
                id: p.id,
                name: p.name,
                category: p.category || '',
                brand: inferBrand(p),
...
is_premium: p.is_premium || 0,
                is_hot: p.is_hot || 0,
                gift: p.gift || ''
            })).filter(p => p.brand !== 'ELECTROLUX' && !/electrolux/i.test(p.name));
            renderAllSections();
        }
    } catch (err) {
...
function updateCustomerAuthUI() {
    ...
    if (accountEl) {
        if (user && getCustomerToken()) {
            accountEl.innerHTML = `<i class="fas fa-user"></i> <span>${escapeHtml(user.fullname || 'Khách Hàng')}</span>`;
            accountEl.href = '#';
        } else {
            accountEl.innerHTML = '<i class="fas fa-sign-in-alt"></i> <span>Đăng nhập</span>';
...
function getAirConditionerHoverHTML(p) {
    if (p.category !== 'Điều Hòa') return '';
    return `
        <div class="aircon-pair" aria-hidden="true">
            <div class="aircon-pair-part aircon-indoor">
                <img src="${escapeHtml(p.image)}" alt="">
                <span>Dàn lạnh</span>
            </div>
            <div class="aircon-pair-part aircon-outdoor">
                <img src="${AIR_CONDITIONER_OUTDOOR_IMAGE}" alt="">
                <span>Dàn nóng</span>
            </div>
        </div>`;
}
...
const INLINE_PRODUCT_BANNERS = [
    { category: 'TV', eyebrow: 'Không gian giải trí đỉnh cao', title: 'SMART TIVI MÀN HÌNH LỚN', note: 'QLED • OLED • Mini LED • Hình ảnh 4K', image: '/assets/images/products/gia-khang/010-google-tivi-tcl-qd-mini-led-4k-115-inch-115x955.jpg', theme: 'blue' },
    { category: 'Tủ Lạnh', eyebrow: 'Giữ trọn tươi ngon', title: 'TỦ LẠNH INVERTER CAO CẤP', note: 'Dung tích lớn • Khử mùi • Tiết kiệm điện', image: '/assets/images/banner-tu-lanh-inverter-khuyen-mai.png?v=1', theme: 'green', fullArtwork: true },
    { category: 'Điều Hòa', eyebrow: 'Giải nhiệt tức thì', title: 'MÁY LẠNH INVERTER', note: 'Làm lạnh nhanh • Êm ái • Tiết kiệm điện', image: '/assets/images/banner-may-lanh-inverter-khuyen-mai.png?v=1', theme: 'teal', fullArtwork: true },
    { category: 'Máy Giặt', eyebrow: 'Giải pháp kinh doanh', title: 'MÁY GIẶT SẤY THƯƠNG MẠI', note: 'Bền bỉ • Tiết kiệm • Vận hành liên tục', image: '/assets/images/banner-may-giat-say-thuong-mai.png?v=1', theme: 'purple', fullArtwork: true },
    { category: 'Máy Giặt', eyebrow: 'Giặt nhiều hơn mỗi mẻ', title: 'MÁY GIẶT DUNG TÍCH LỚN', note: 'Dễ sử dụng • Lồng giặt bền • Nhiều chương trình', image: '/assets/images/banner-may-giat-dung-tich-lon-moi.png?v=1', theme: 'orange', fullArtwork: true },
    { category: 'TV', eyebrow: 'Công nghệ hình ảnh mới', title: 'GOOGLE TIVI 4K SONY', note: 'Màu sắc trung thực • Âm thanh sống động', image: '/assets/images/products/gia-khang/175-google-tivi-true-rgb-sony-ai-4k-75-inch-k-75xr70m2.webp', theme: 'red' }
];

function getInlineProductBannerHTML(index) {
    const banner = INLINE_PRODUCT_BANNERS[index % INLINE_PRODUCT_BANNERS.length];
    return `
        <button type="button" class="inline-product-banner theme-${banner.theme}${banner.fullArtwork ? ' full-artwork' : ''}" onclick="openInlineProductBanner('${banner.category}')">
            <span class="inline-banner-copy">
                <small>${banner.eyebrow}</small>
                <strong>${banner.title}</strong>
                <span>${banner.note}</span>
                <b>Xem sản phẩm <i class="fas fa-arrow-right"></i></b>
            </span>
            <img src="${banner.image}" alt="${banner.title}" loading="lazy">
        </button>`;
}

function openInlineProductBanner(category) {
...
function renderProducts(brand = 'all', search = '', category = '') {
    ...
    // Render tạm để trình duyệt tính đúng số cột, sau đó chèn 1 banner mỗi 3 hàng.
    grid.innerHTML = filtered.map(p => getProductCardHTML(p)).join('');
    const renderedColumns = getComputedStyle(grid).gridTemplateColumns.split(/\s+/).filter(Boolean).length || 1;
    const productsPerBanner = renderedColumns * 3;
    const content = [];
    let bannerIndex = 0;
    filtered.forEach((product, index) => {
        content.push(getProductCardHTML(product));
        const completedBlock = (index + 1) % productsPerBanner === 0;
        if (completedBlock && index < filtered.length - 1) {
            content.push(getInlineProductBannerHTML(bannerIndex++));
        }
    });
...
const PREMIUM_CATEGORIES = [
    ...
    { key: 'maylanh', label: 'Tủ lạnh cao cấp', category: 'Tủ Lạnh', mode: 'category', icon: 'fa-snowflake' },
...
const PREMIUM_CAT_IMAGES = {
    ...
    maylanh: '/assets/images/products/gia-khang/055-tu-lanh-lg-inverter-603-lit-multi-door-f53egr.webp',
...
<div class="premium-p-top">
            ...
            <div class="premium-p-body">
                <div class="premium-p-cat">${p.category}</div>
                ...
                html += `
                    <div class="search-dropdown-item" data-product-id="${p.id}">
                        ...
                        <div class="search-dropdown-item-info">
                            ...
                            <div class="search-dropdown-item-desc">${highlightText(p.category, input.value)} - ${formatCurrency(p.price)}</div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    initHomeHeroSlider();
    // Phân bổ banner theo nhịp trang: đầu, giữa và cuối khu sản phẩm.
    const middleShowcase = document.getElementById('showcaseMiddle');
    const bottomShowcase = document.getElementById('showcaseBottom');
    const airBanner = document.querySelector('.showcase-air');
    const frontWasherBanner = document.querySelector('.showcase-washer-front');
    const topWasherBanner = document.querySelector('.showcase-washer-top');
    if (middleShowcase && airBanner && frontWasherBanner) {
        middleShowcase.append(airBanner, frontWasherBanner);
    }
    if (bottomShowcase && topWasherBanner) {
        bottomShowcase.append(topWasherBanner);
    }

    document.querySelectorAll('[data-showcase-category]').forEach(card => {
        card.addEventListener('click', function() {
            const category = this.dataset.showcaseCategory || '';
            syncBrandFilters('all');
            renderProducts('all', '', category);
            document.getElementById('products-section')?.scrollIntoView({ behavior: 'smooth' });
        });
